...
JNIEXPORT jobject JNICALL
Java_com_tryfit_camera_PaperDetectionActivity_findFootOnSimplePictureAndroid(JNIEnv *env,
                                                                                     jclass type,
                                                                                     jlong inputMatAddr,
                                                                                     jlong outputMatAddr) {
    ...
    jclass tryFitLibReturnClass = env->FindClass(
            "com/tryfit/camera/TryFitLibResult");
    jmethodID constructor = env->GetMethodID(tryFitLibReturnClass, "<init>", "(II)V");
    jobject tryFitLibReturnObject = env->NewObject(tryFitLibReturnClass, constructor, type, result);
...
JNIEXPORT jobject JNICALL
Java_com_tryfit_camera_CameraFragment_findFootOnSimplePictureAndroid2(JNIEnv *env,
                                                                      jclass type,
                                                                      jint width,
                                                                      jint height,
                                                                      jbyteArray NV21FrameData,
                                                                      jintArray outPixels2,
                                                                      jboolean isFromGallery) {



    jbyte *_yuv = env->GetByteArrayElements(NV21FrameData, 0);
    jint *_bgra2 = env->GetIntArrayElements(outPixels2, 0);
    ...
    Mat outMat(height, width, CV_8UC4);

    if (isFromGallery) {
        Mat myuv(height, width, CV_8UC3, (uchar *) _yuv);
        inMat = imdecode(myuv, CV_LOAD_IMAGE_COLOR);
        cvtColor(inMat, inMat, CV_RGB2RGBA);
    } else {
        Mat myuv(height + height / 2, width, CV_8UC1, (uchar *) _yuv);
        cvtColor(myuv, inMat, CV_YUV420sp2BGRA);
    }
...
//    cvtColor(outMat,outMat,CV_BGRA2BGR);
    _bgra2 = (jint *) outMat.data;
    ...
    LOGI(TAG, "FINISH JJ: %d %d", inMat.cols, inMat.rows);

    env->ReleaseIntArrayElements(outPixels2, _bgra2, JNI_COMMIT);
